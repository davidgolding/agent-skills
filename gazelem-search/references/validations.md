...
### **Message**
Vector mismatch: Attempted to run text-based search (grep, regex, or substring search) on semantic_cache.toon. Mode B must strictly use vector similarity matching.
### **Fix Action**
Compute the query vector embedding, and compute similarity scores against the float vectors stored in semantic_cache.toon to extract the closest candidate segment IDs.
### **Applies To**
- *.md
- *.py
- *.sh
