...
Include a visual aid when the requirements would be significantly easier to understand with one. Read `references/visual_communication.md` for the decision criteria, format selection, and placement rules.
...
- What would the agent still have to invent to draft/write the skill if this brainstorm ended now?
...
Ensure the workspace root is writeable before writing the temporary requirements file.
