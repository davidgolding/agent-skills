...
- **Naming implementation detail in any bullet**: file paths, module names, exact JSON keys, HTTP status codes, error message wording, SQL syntax. The synthesis is scope-only; implementation is the coding phase's job. These granularity rules apply to every bullet in every section.
...
- "Use Postgres for persistence" — implementation detail, failed granularity rules
...
If the user response indicates they want a different workflow:
...
The doc's `## Summary` and `## Problem Frame` must serve distinct purposes — see `references/requirements_capture.md` "Summary vs Problem Frame discipline" for the rules.
